...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        validate_procedure(operator) #check the operator
        args = rest.map(lambda x: scheme_eval(x,env))
        return scheme_apply(operator,args,env) #return expressions and evaluates special forms
        # END PROBLEM 4
...
def eval_all(expressions, env):
    ...
    while expressions is not nil:
        result=scheme_eval(expressions.first, env) # replace this with lines of your own code
        expressions=expressions.rest #going to the next expression
    return result
    
    #Expressions are pair. It's a linked list.
    # END PROBLEM 7
...
class Frame(object):
    ...
    def define(self, symbol, value):
        ...
        # BEGIN PROBLEM 2
        "*** YOUR CODE HERE ***"
        self.bindings[symbol] = value #assign a value to a key
        # END PROBLEM 2
    ...
    def lookup(self, symbol):
        ...
        if symbol in self.bindings:
            return self.bindings[symbol] # return a value in dictionary
    ...
    def make_child_frame(self, formals, vals):
        ...
        "*** YOUR CODE HERE ***"
        child=Frame(self)
        while formals is not nil and vals is not nil: #nil is a Class
            child.define(formals.first,vals.first) #bind formal parameters to value
            formals,vals = formals.rest, vals.rest #update the formals
        return child
        # END PROBLEM 10
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        4
        """
        if not scheme_listp(args): #args is a Scheme list represented as Pair
        ...
        while args is not nil:
            python_args.append(args.first) #append the first item in the list
            args = args.rest #update the args
        if self.use_env:
            python_args.append(env) #env is not a list

        # END PROBLEM 3
        try: #so when you run a code one error doesn't stop the process
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 11
        "*** YOUR CODE HERE ***"
        return self.env.make_child_frame(self.formals,args) #call make_child_frame on the frame where the Lambda procedure is defined
...
def do_define_form(expressions, env): #the structure of the exp is Pair(A, Pair(B, nil))
    ...
    target = expressions.first # Expression is a linked list
    if scheme_symbolp(target):
        validate_form(expressions, 2, 2) # Checks that expressions is a list of length exactly 2
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        result=scheme_eval(expressions.rest.first,env) #evaluate the expression
        env.define(target,result) #binds target to result
        return target
        # END PROBLEM 5
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 9
        # Creates a lambda procedure and name it
        # Original: (define (f x y) (+ x y)) -- (x y) is arguments
        # Modify to: (lambda (x y) (+ x y))
        "*** YOUR CODE HERE ***"
        # Break down the expressions to formals and body: Pair(target.rest,expression.rest)
        result=do_lambda_form(Pair(target.rest,expressions.rest), env) #create the LambdaProcedure instance
        env.define(target.first,result) # bind the name to the LambdaProcedure instance
        return target.first # return the name
        # END PROBLEM 9
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))

def do_quote_form(expressions, env): #the structure of expression is Pair(A, nil)
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***" 
    return expressions.first #return unevaluated operands
    # END PROBLEM 6
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 8
    # formals is the parameter names
    # body is the list of expressions to evaluate
    # env is the frame in which the expression is evaluated
    "*** YOUR CODE HERE ***"
    return LambdaProcedure(formals, expressions.rest,env) #returning an instance of LambdaProcedure
    # END PROBLEM 8
...
def do_if_form(expressions, env):
    ...
    3
    """
    validate_form(expressions, 2, 3) #checks if the expression has a length between 2 and 3
    if is_true_primitive(scheme_eval(expressions.first, env)): #evaluates the condition, check if it is a Truthy
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_true_primitive(result): #evaluates the first sub-expression, check if it is a Truthy
            expressions = expressions.rest #keep going, no need to update the result
        else: # put else otherwise it'll return the next line directly
...
def do_or_form(expressions, env):
    ...
    # BEGIN PROBLEM 12
    "*** YOUR CODE HERE ***"
    result = False #OR is defaulted to false
    while expressions is not nil:
        result = scheme_eval(expressions.first, env) #define the result
        if is_false_primitive(result): #evaluates the first sub-expression, check if it is a Truthy
            expressions = expressions.rest #keep going, no need to update the result
        else: # put the "else" otherwise it'll go to the next line immediately
            return result #result has to be a boolean
    return result 
...
def do_cond_form(expressions, env):
    ...
    """
    while expressions is not nil:
        clause = expressions.first #clause contains predicate and expressions
        validate_form(clause, 1)
        if clause.first == 'else': #can be a predicate or 'else'
            test = True
            if expressions.rest != nil:
                raise SchemeError('else must be last')
        else:
            test = scheme_eval(clause.first, env) #evaluate the predicate and save the value in test
        if is_true_primitive(test):
            # BEGIN PROBLEM 13
            "*** YOUR CODE HERE ***"
            if clause.rest is nil: #if there is no expression
                return test #return the predicate
            return eval_all(clause.rest,env) 
        
            # END PROBLEM 13
        expressions = expressions.rest #moving on to the next sub-expression
...
def do_let_form(expressions, env):
    ...
    #let_form makes a temporary expression
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        validate_form(bindings.first,2,2) #make sure the expression length is always 2
        result=scheme_eval(bindings.first.rest.first,env) #evaluate the bindings
        # names, values = result.first, result.rest #update names and values
        names = Pair(bindings.first.first,names)
        values = Pair(result,values) # add on of other values
        bindings = bindings.rest # move on to the next pair of bindings
    validate_formals(names) #validate names is a scheme list
    # END PROBLEM 14
