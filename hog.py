...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0<goal and score1<goal:  
        if who==0:
            ...
            strategy=strategy0(player_score,opponent_score)
            take_turn_value=take_turn(strategy,opponent_score,dice)
            score0+=take_turn_value
            extra_turn_value=extra_turn(score0,score1)

        else:
            player_score=score1
            opponent_score=score0
            strategy=strategy1(player_score,opponent_score)
            take_turn_value= take_turn(strategy,opponent_score,dice)
            score1+=take_turn_value
            extra_turn_value=extra_turn(score1,score0)

        if not extra_turn_value:
            who=other(who)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        if current_score-last_score>running_high:
            print(current_score-last_score,'point(s)! The most yet for Player',who)
            return announce_highest(who,current_score,current_score-last_score)
...
def make_averaged(original_function, trials_count=1000):
    ...
    def func(*args):
        ...
        while count <= trials_count:
            storage = original_function(*args)
            total+= storage
            count += 1
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    "*** YOUR CODE HERE ***"
    num_rolls=1
    result=0 # storage for highest average
    average=0 #output of the make_average function
    count=0
    while num_rolls <=10:
